...
struct MissedDayDialog: View {
    ...
    // MARK: - Actions
    // Order is intentional: mark handled BEFORE any state mutation so an inadvertent
    // re-render of TodayView can't re-trigger the shouldShow check.
    ...
    static func shouldShow(state: AppState) -> Bool {
        ...
        // Don't show on the user's very first day — there's nothing to have missed.
        let daysIn = state.dayNumber
        guard daysIn >= 1 else { return false }
